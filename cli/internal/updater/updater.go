...
type CheckResult struct {
	...
	LastNotifyAt  int64  `json:"lastNotifyAt"` // last time an update notice was printed
}
...
func IsNewer(current, latest string) bool {
	_, _, _, _, currentOK := parseVersion(current)
	_, _, _, _, latestOK := parseVersion(latest)
...
func CheckAsync(currentVersion string) {
	go func() {
		...
		// Rate-limit the async notice to once per day.
		if result.LastNotifyAt > 0 && time.Since(time.Unix(result.LastNotifyAt, 0)) < cacheTTL {
			return
		}
